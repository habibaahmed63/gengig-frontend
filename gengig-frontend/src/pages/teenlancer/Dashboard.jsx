...
export default function TeenlancerDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        const [activeRes, completedRes, statsRes, appsRes] = await Promise.all([
  ...
  const hasActivity = activeGigs.length > 0 || completedGigs.length > 0 || myApplications.length > 0;
            ...
            <div key={stat.label}
              ...
              <div>
                ...
                <p className="font-bold text-xl"
                  style={{ color: stat.value === "—" || stat.value === "$0" || stat.value === 0 ? "#B2B2D2" : "white" }}>
                  {stat.value}
                </p>
              </div>
                ...
                {myApplications.map((app) => {
                  ...
                  return (
                    <div key={app.id}
                      ...
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm font-semibold truncate mb-0.5">{app.gigTitle || "Untitled Gig"}</p>
                        <p className="text-xs" style={{ color: "#B2B2D2" }}>
                          by {app.agentName || "—"}
                          {app.appliedAt && " · Applied " + new Date(app.appliedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                        </p>
                      </div>
                        ...
                        {app.status === "accepted" && (
                          <button
                            onClick={() => navigate("/teenlancer/chat", {
                              state: { openContact: { id: app.agentId, name: app.agentName, photo: app.agentPhoto } }
                            })}
