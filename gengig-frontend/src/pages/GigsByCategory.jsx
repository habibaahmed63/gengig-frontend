...
export default function GigsByCategory() {
    ...
    const handleSave = async (gigId) => {
        const isCurrentlySaved = savedGigs.includes(gigId);
        setSavedGigs(prev =>
            isCurrentlySaved ? prev.filter(id => id !== gigId) : [...prev, gigId]
        );
        try {
            ...
            setSavedGigs(prev =>
                isCurrentlySaved ? [...prev, gigId] : prev.filter(id => id !== gigId)
            );
        }
    };
    ...
    const sorted = [...filtered].sort((a, b) => {
        if (sort === "budget_high") return (parseFloat(String(b.budget).replace(/[^0-9.]/g, "")) || 0) - (parseFloat(String(a.budget).replace(/[^0-9.]/g, "")) || 0);
        if (sort === "budget_low") return (parseFloat(String(a.budget).replace(/[^0-9.]/g, "")) || 0) - (parseFloat(String(b.budget).replace(/[^0-9.]/g, "")) || 0);
        if (sort === "popular") return (b.applications ?? 0) - (a.applications ?? 0);
        return 0;
    });
    ...
    return (
        <TeenlancerLayout>
            ...
            {/* Hero */}
            <div className="p-8 rounded-2xl mb-8 relative overflow-hidden"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[...Array(6)].map((_, i) => (
                        <div key={i} className="p-5 rounded-2xl animate-pulse"
                            style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", height: "160px" }}>
