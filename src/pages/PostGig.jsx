...
export default function PostGig() {
    ...
    const [form, setForm] = useState({
        ...
        customBudget: "", deadline: "", skillsInput: "",
    ...
    const setReq = (i, val) => { const r = [...form.requirements]; r[i] = val; set("requirements", r); };
    ...
    const validate = (s) => {
        ...
        if (s === 1) {
            ...
            if (form.budget === "Custom" && !form.customBudget.trim()) e.customBudget = "Please enter a custom budget.";
        }
        setErrors(e);
                    ...
                    <div className="xl:col-span-1 flex flex-col gap-5">
                                ...
                                {STEPS.map((s, i) => (
                                    <div key={s} className="flex items-center gap-3">
                                        <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all"
                                            style={{
                                                background: i < step ? "linear-gradient(90deg, #FFC085, #e8a060)" : i === step ? "rgba(255,192,133,0.2)" : "rgba(255,255,255,0.06)",
                        ...
                        <div className="p-5 rounded-2xl"
                            ...
                            <div className="p-4 rounded-xl"
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {form.deadline && <span className="text-xs" style={{ color: "#B2B2D2" }}>⏱ {form.deadline}</span>}
                                </div>
                            </div>
                        ...
                        <div className="p-5 rounded-2xl"
                            ...
                            <ul className="flex flex-col gap-2">
                                {["Write a clear, specific title", "Set a realistic budget for quality work", "List required skills precisely", "Break requirements into bullet points"].map(tip => (
                    ...
                    <div className="xl:col-span-2">
                        <div className="p-6 sm:p-8 rounded-2xl"
                                ...
                                <div className="flex flex-col gap-6">
                                    <div className="flex flex-col gap-2">
                                        <div className="flex justify-between">
                                            <label className="text-sm font-medium text-white">Gig Title <span style={{ color: "#f87171" }}>*</span></label>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-sm font-medium text-white">Category <span style={{ color: "#f87171" }}>*</span></label>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <div className="flex justify-between">
                                            <label className="text-sm font-medium text-white">Description <span style={{ color: "#f87171" }}>*</span></label>
                                        ...
                                        <p className="text-xs" style={{ color: "#B2B2D2" }}>Describe what you need, the style you're going for, and any important details.</p>
                                        <textarea value={form.description}
                                            onChange={e => set("description", e.target.value.slice(0, FIELD_LIMITS.description))}
                                            placeholder="We are looking for a talented designer to create a complete brand identity..."
                                            rows={5}
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="w-full rounded-xl px-3 py-2 flex flex-wrap gap-2 min-h-[48px] cursor-text"
                                            style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }}
                                            onClick={() => document.getElementById("skills-input")?.focus()}>
                                            {form.skills.map(skill => (
                                                <span key={skill} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium"
                                            ...
                                            {form.skills.length < 10 && (
                                                <input id="skills-input" type="text" value={form.skillsInput}
                            ...
                            {step === 1 && (
                                <div className="flex flex-col gap-6">
                                    <div className="flex flex-col gap-2">
                                        <label className="text-sm font-medium text-white">Budget Range <span style={{ color: "#f87171" }}>*</span></label>
                                        ...
                                        {form.budget === "Custom" && (
                                            <div className="relative mt-2">
                                                ...
                                                <input type="number" value={form.customBudget}
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label className="text-sm font-medium text-white">Project Deadline <span style={{ color: "#f87171" }}>*</span></label>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            {DEADLINES.map(d => (
                                                <button key={d.value} onClick={() => set("deadline", d.value)}
                                                    className="py-3 px-4 rounded-xl text-sm font-medium transition-all hover:opacity-90"
                                                    style={{
                                    ...
                                    {form.budget && form.deadline && (
                                        <div className="p-4 rounded-xl flex items-center gap-4"
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-white font-bold">{form.budget === "Custom" ? (form.customBudget ? `$${form.customBudget}` : "—") : form.budget}</p>
                                            </div>
                            ...
                            {step === 2 && (
                                <div className="flex flex-col gap-6">
                                    <div className="flex flex-col gap-3">
                                        ...
                                        <p className="text-xs" style={{ color: "#B2B2D2" }}>List specific things the teenlancer must deliver.</p>
                                        {form.requirements.map((req, i) => (
                                            <div key={i} className="flex items-center gap-2">
                                                <span className="text-xs font-bold w-5 text-center flex-shrink-0" style={{ color: "#FFC085" }}>{i + 1}</span>
                                                <input type="text" value={req} onChange={e => setReq(i, e.target.value)}
                                                ...
                                                {form.requirements.length > 1 && (
                                                    <button onClick={() => removeReq(i)}
                                                        className="w-8 h-8 rounded-full flex items-center justify-center text-xs hover:opacity-80 flex-shrink-0"
                                                        style={{ background: "rgba(248,113,113,0.1)", color: "#f87171" }}>✕</button>
                                                )}
                                            </div>
                                        ))}
                                        {form.requirements.length < 8 && (
                                            <button onClick={addReq} className="flex items-center gap-2 text-sm hover:opacity-80 self-start" style={{ color: "#FFC085" }}>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <textarea value={form.notes || ""} onChange={e => set("notes", e.target.value)}
                                            placeholder="Any other information teenlancers should know..."
                                            rows={3}
                                            className="w-full rounded-xl px-4 py-3 text-white text-sm outline-none focus:ring-1 focus:ring-[#FFC085] resize-none"
                                            style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)" }} />
                                    </div>
                            ...
                            {step === 3 && (
                                <div className="flex flex-col gap-5">
                                    <p className="text-sm" style={{ color: "#B2B2D2" }}>Review everything before posting. You can go back to edit any section.</p>
                                    {[
                                        {
                                            title: "Gig Details", step: 0, items: [
                                        ...
                                        {
                                            title: "Budget & Timeline", step: 1, items: [
                                        ...
                                        <div key={section.title} className="p-5 rounded-2xl"
                                            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                                            <div className="flex items-center justify-between mb-4">
                                                <h3 className="text-white font-semibold text-sm">{section.title}</h3>
                                                <button onClick={() => setStep(section.step)} className="text-xs hover:opacity-80" style={{ color: "#FFC085" }}>Edit</button>
                                            </div>
                                            <div className="flex flex-col gap-3">
                                    ...
                                    <div className="p-4 rounded-xl flex gap-3"
                                        ...
                                        <p className="text-xs leading-relaxed" style={{ color: "#B2B2D2" }}>
                                            ...
                                            <button onClick={() => window.open("/Terms", "_blank")} className="hover:opacity-80" style={{ color: "#FFC085" }}>
                                                Terms of Service
                                            </button>. A small service fee applies on completed transactions.
                                        </p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className={`flex gap-3 mt-8 ${step > 0 ? "justify-between" : "justify-end"}`}>
                                ...
                                {step < STEPS.length - 1 ? (
                                    ...
                                    <button onClick={handleSubmit} disabled={loading}
                                        className="px-8 py-2.5 rounded-full text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50 flex items-center gap-2"
                                        style={{ background: "linear-gradient(90deg, #FFC085, #e8a060)" }}>
                                        {loading ? (
                                            <><div className="w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: "white", borderTopColor: "transparent" }} /> Posting...</>
                                        ) : "Post Gig"}
                                    </button>
                                )}
                            </div>
